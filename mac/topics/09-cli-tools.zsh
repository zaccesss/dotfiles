...
# all keep working exactly as before. Install:
#   brew install fzf zoxide bat eza ripgrep
# =============================================================================
...
# replace: in-place find-and-replace in a file, BSD sed (macOS) needs the extra ''
replace() {
...
# notify: run a command, send a macOS notification with its exit status when done.
# I use this for a build or long-running script I want to walk away from.
notify() {
    "$@"
    local status=$?
